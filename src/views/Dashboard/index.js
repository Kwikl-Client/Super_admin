...
const Default = () => {
  ...
  const [reveuneMade, setRevenueMade] = useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const apiRevenue = 'http://localhost:7000/payment/totalRevenue';
        const apiCustomers = 'http://localhost:7000/customer/getAllCustomer';

        // Make both API calls in parallel using Promise.all
        const [revenueResponse, customerResponse] = await Promise.all([
          axios.get(apiRevenue),
          axios.post(apiCustomers),
        ]);

        // Handle the responses
        setRevenueMade(revenueResponse.data);
        setDashboardCount(customerResponse.data.data.totalCustomers);
    ...
    // Call the function to initiate the API calls
    fetchData();
  }, []);
  return (
    <Grid container spacing={gridSpacing}>
      <Grid item xs={12}>
        ...
        <Grid item lg={3} sm={6} xs={12}>
          <ReportCard
            primary={reveuneMade?.data?reveuneMade.data:"Loading..."}
        ...
        <Grid item lg={3} sm={6} xs={12}>
          <ReportCard
            primary={dashboardCount?dashboardCount:"Loading..."}
            secondary="Users"
